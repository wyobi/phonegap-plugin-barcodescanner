...
public class BarcodeScanner extends CordovaPlugin {
    ...
    private static final String RETICLE_STYLE = "reticleStyle";
            ...
            public void run() {
                ...
                if (args.length() > 0) {
                    ...
                    for (int i = 0; i < args.length(); i++) {
                        ...
                        int cameraId = obj.optBoolean(PREFER_FRONTCAMERA, false) ? 1 : 0;
                        if(obj.has(CAMERAID) && !obj.isNull(CAMERAID)) {
                            try {
                                cameraId = obj.getInt(CAMERAID);
                            }
                            catch (JSONException e) {
                                e.printStackTrace();
                            }
                        ...
                        // Handle reticleStyle: "square" or "auto"/"rectangle" (default)
                        // Only "square" sets explicit dimensions - rectangle/auto use default
                        // which adapts to orientation (important for PDF417 in landscape)
                        if (!obj.has(WIDTH) && !obj.has(HEIGHT) && obj.has(RETICLE_STYLE)) {
                            String reticleStyle = obj.optString(RETICLE_STYLE, "auto");
                            if ("square".equals(reticleStyle)) {
                                // Square reticle for QR codes and DataMatrix (60% of smaller dimension)
                                android.util.DisplayMetrics dm = that.cordova.getActivity().getResources().getDisplayMetrics();
                                int size = (int) (Math.min(dm.widthPixels, dm.heightPixels) * 0.6);
                                intentScan.putExtra(Intents.Scan.WIDTH, size);
                                intentScan.putExtra(Intents.Scan.HEIGHT, size);
                                Log.i(LOG_TAG, "Using square reticle: " + size + "x" + size);
                            }
                            // "rectangle" and "auto" use the default orientation-aware framing
                            // Default: 5/8 of each dimension, max 1600x900
                            // This automatically creates wide viewfinder in landscape for PDF417
                        }
                        ...
                        if (obj.has(FORMATS)) {
                            String formats = obj.optString(FORMATS);
                            intentScan.putExtra(Intents.Scan.FORMATS, formats);
                            Log.i(LOG_TAG, "=== SCAN REQUESTED ===");
                            Log.i(LOG_TAG, "Formats: " + formats);
                            Log.i(LOG_TAG, "TryHarder: " + obj.has("tryHarder"));
                            Log.i(LOG_TAG, "ReticleStyle: " + obj.optString(RETICLE_STYLE, "auto"));
                        }
                        if (obj.has(PROMPT)) {
